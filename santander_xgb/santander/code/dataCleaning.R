...
{setwd('~/dataScience/workspace/santander/code')}
...
trainPeriods = function() {
	
	df <- fread("../input/train_ver2.csv",nrows=-1)
	
	unique(df$fecha_dato)
	
	trainPeriodsag2015=filter(df, fecha_dato == '2015-01-28')
...
# gradient boosting classifier **  log loss and confusion matrix  
#https://www.kaggle.com/raddar/predicting-red-hat-business-value/0-98-xgboost-on-sparse-matrix/code
...
options(digits=2)
require(Hmisc)
bystats(df_e$renta, df_e$segmento, 
